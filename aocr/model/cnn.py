...
def max_3x1pool(incoming, name):
    ...
    '''
    with tf.variable_scope(name):
        return tf.nn.max_pool(incoming, ksize=(1, 3, 1, 1), strides=(1, 3, 1, 1), padding='VALID')


def ConvRelu(incoming, num_filters, filter_size, name, padding="SAME"):
    '''
    ...
    with tf.variable_scope(name):
        ...
        after_conv = tf.nn.conv2d(incoming, conv_W, strides=(1, 1, 1, 1), padding=padding)
...
class CNN(object):
    ...
    def _build_network(self, input_tensor, is_training):
        ...
        net = ConvRelu(net, 64, (3, 3), 'conv_conv1', padding="SAME")
        ...
        net = max_2x1pool(net, 'conv_pool2')

        net = ConvRelu(net, 256, (3, 3), 'conv_conv5')
        net = ConvReluBN(net, 256, (3, 3), 'conv_conv6', is_training)
        net = max_2x2pool(net, 'conv_pool3')


        net = ConvRelu(net, 512, (3, 3), 'conv_conv7')
        net = ConvReluBN(net, 512, (3, 3), 'conv_conv8', is_training)

        net = max_3x1pool(net, 'conv_pool4')
